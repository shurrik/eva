 ...
 * - `{void}` `put({string} key, {*} value)` 钬?Puts a new key-value pair into the cache.
 ...
 * **Methods**
 *
 * - `then(successCallback, errorCallback)` 钬?regardless of when the promise was or will be resolved
...
function $RouteProvider(){
   ...
   *    Object properties:
   *
   *    - `controller` 钬?`{(string|function()=}` 钬?Controller fn that should be associated with newly
   *      created scope or the name of a {@link angular.Module#controller registered controller}
   *      if passed as a string.
   *    - `template` 钬?`{string=}` 钬? html template as a string that should be used by
   *      {@link ng.directive:ngView ngView} or
   *      {@link ng.directive:ngInclude ngInclude} directives.
   *      this property takes precedence over `templateUrl`.
   *    - `templateUrl` 钬?`{string=}` 钬?path to an html template that should be used by
...
function $HttpProvider() {
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     *
     * # General usage
     * The `$http` service is a function which takes a single argument 钬?a configuration object 钬?
     ...
     * the `then` method to register callbacks, and these callbacks will receive a single argument 钬?
     ...
     *    - **params** 钬?`{Object.<string|Object>}` 钬?Map of strings or objects which will be turned to
     *      `?key1=value1&key2=value2` after the url. If the value is not a string, it will be JSONified.
     *    - **data** 钬?`{string|Object}` 钬?Data to be sent as the request message data.
     *    - **headers** 钬?`{Object}` 钬?Map of strings representing HTTP headers to send to the server.
     *    - **transformRequest** 钬?`{function(data, headersGetter)|Array.<function(data, headersGetter)>}` 钬?
     *      transform function or an array of such functions. The transform function takes the http
     *      request body and headers and returns its transformed (typically serialized) version.
     *    - **transformResponse** 钬?`{function(data, headersGetter)|Array.<function(data, headersGetter)>}` 钬?
     *      transform function or an array of such functions. The transform function takes the http
     ...
     *      caching.
     *    - **timeout** 钬?`{number}` 钬?timeout in milliseconds.
     ...
     *
     *   - **data** 钬?`{string|Object}` 钬?The response body transformed with the transform functions.
 ...
 *  forms, where:
 *
 *  - keys are validation tokens (error names) 钬?such as `required`, `url` or `email`),
 ...
 *
 * * Model 钬?The Model is data in scope properties; scopes are attached to the DOM.
 * * View 钬?The template (HTML with data bindings) is rendered into the View.
 * * Controller 钬?The `ngController` directive specifies a Controller class; the class has
 ...
 *
 *   * `$index` 钬?`{number}` 钬?iterator offset of the repeated element (0..length-1)
 *   * `$first` 钬?`{boolean}` 钬?true if the repeated element is first in the iterator.
 *   * `$middle` 钬?`{boolean}` 钬?true if the repeated element is between the first and last in the iterator.
 *   * `$last` 钬?`{boolean}` 钬?true if the repeated element is last in the iterator.
 *
 *
 * @element ANY
 * @scope
